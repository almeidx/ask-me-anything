...
import (
	...
	"github.com/almeidx/ask-me-anything/internal/store/pgstore"
...
func (h apiHandler) notifyClients(msg Message) {
	...
	for conn, cancel := range subscribers {
		if err := conn.WriteJSON(msg); err != nil {
			slog.Warn("failed to send message to subscriber", "error", err)
			cancel()
		}
	}
}
...
func (h apiHandler) handleSubscribe(w http.ResponseWriter, r *http.Request) {
	rawRoomId, _, err := getRoomIDFromRequest(w, r, h)
	if err != nil {
		return
	}
	...
	if err != nil {
		...
		http.Error(w, "failed to upgradde to ws connection", http.StatusInternalServerError)
		return
	}
...
func (h apiHandler) handleCreateRoomMessage(w http.ResponseWriter, r *http.Request) {
	rawRoomId, roomId, err := getRoomIDFromRequest(w, r, h)
	if err != nil {
		return
	}
